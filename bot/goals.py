...
def daily_goal_progress_line(tech_id: str) -> str:
...
def weekly_goal_progress_line(tech_id: str) -> str:
    today = miami_now().date()
    week_start, _ = week_bounds(today)
...
def goals_progress_block(tech_id: str) -> str:
    lines = [line for line in (daily_goal_progress_line(tech_id), weekly_goal_progress_line(tech_id)) if line]
    return "\n".join(lines)
