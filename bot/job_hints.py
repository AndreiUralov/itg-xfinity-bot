...
def lookup_job_hint(job_number: str | int, owner_telegram_id: int | None = None) -> dict[str, Any] | None:
    """Return typical work type and amount for a job number from history."""
    jn = str(job_number).strip()
    if not jn:
        return None

    rows = [r for r in read_all_rows(owner_telegram_id) if str(r.get("job_number")) == jn]
    if not rows:
        return None
